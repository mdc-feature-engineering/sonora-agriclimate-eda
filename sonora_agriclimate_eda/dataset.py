...
def download_drought_data( start_year: int = 2019, end_year: int = 2024 ):
    ...
    try:
        raw_path.write_bytes(requests.get(url, timeout=60).content)
...
def download_rain_data(start_year: int = 2019, end_year: int = 2024):
    ...
    main_folder = RAW_DATA_DIR # os.path.join("..", "data", "raw", "datos_lluvia_2019_2023")
        ...
        for month in range(1, 13):
            ...
            try:
                ...
                if res.status_code == 200: 
                    ...
                    logger.warning(f"Año {year_str} | Mes {mes_str}: No disponible (Status {res.status_code})")
            except Exception as e:
